...
%   See also ALPHA_SHAPE, ALPHA_SHAPE
    ...
    number_of_all_regions = numRegions(shp);
    number_of_regions = sum(area(shp, 1:number_of_all_regions ) > minimum_defective_area);
    % encoded_shape_map = zeros(1,number_of_regions);
    bounding_boxes = zeros(2,2,number_of_regions);
    encoded_shape_maps = [];
    
    % For each region
    for region_id = 1 : number_of_all_regions
        if area(shp, region_id) > minimum_defective_area
