...
Example:
    ...
    \subsection{Defect analysis}\label{subsection:defects}
        ...
        \subsubsection{Defect class \#1}\label{section:defect-class-3}
            ...
            \end{figure}
            \begin{figure}
                % \includegraphics[width=\linewidth]{graphics/defects/class1gaussian}
                \caption{Shape size bivariate lognormal distribution for defect class \#1}\label{fig:defects:gaussian-1}
            \end{figure}
            \par{
            ...
            \par{
                The bivariate lognormal distribution of the shape size is shown in \ref{fig:defects:gaussian-1}.
            }
        \subsubsection{Defect class \#2}
            ...
            \end{figure}
            \begin{figure}
                % \includegraphics[width=\linewidth]{graphics/defects/class2gaussian}
                \caption{Shape size bivariate lognormal distribution for defect class \#2}\label{fig:defects:gaussian-2}
            \end{figure}
            \par{
                In figure \ref{fig:defects:surface-2} an example of surface with a defect of this type is shown. The shape distribution of the defect is illustrated in picture \ref{fig:defects:shape-2}. The bivariate lognormal distribution of the shape size is shown in \ref{fig:defects:gaussian-2}.
            }
        \subsubsection{Defect class \#3}
            ...
            \end{figure}
            \par{
                Defects of class \#3 are probably rolling defects, in particular their repetitive pattern along the centreline is a symptom of \emph{zipper cracks}, i.e. centre line cracking. This is another patent example of burst defect, indeed it is reasonable to expect such defects to repeat multiple times on the same surface.
            }
            \begin{figure}
            ...
            \end{figure}
            \begin{figure}
                % \includegraphics[width=\linewidth]{graphics/defects/class3gaussian}
                \caption{Shape size bivariate lognormal distribution for defect class \#3}\label{fig:defects:gaussian-3}
            \end{figure}
            \par{
                An example of surface affected by defects of class \#3 is shown in figure \ref{fig:defects:surface-3}. A single defect of class \#3 (i.e. a single crack) has usually a shape as the one shown in picture \ref{fig:defects:shape-3}. The bivariate lognormal distribution of the shape size is shown in \ref{fig:defects:gaussian-3}.
            }

        \subsubsection{Defect class \#4}
            \par{
                This defect class seems to be concerned with protrusions on the steel surface. The main typologies of protuberances in the dataset are \emph{scabs} and \emph{blisters}.
            }
            \begin{figure}
            ...
            \end{figure}
            \begin{figure}
                % \includegraphics[width=\linewidth]{graphics/defects/class4gaussian}
                \caption{Shape size bivariate lognormal distribution for defect class \#4}\label{fig:defects:gaussian-4}
            \end{figure}
            \par{
                An example of surface affected by defects of class \#4 is shown in figure \ref{fig:defects:surface-4}. A single defect of class \#4 (i.e. a single crack) has usually a shape as the one shown in picture \ref{fig:defects:shape-4}. The bivariate lognormal distribution of the shape size is shown in \ref{fig:defects:gaussian-4}. These statistics have been used, along with the visual inspection, to infer the origin of this defect class.
            }
            \par{
    ...
    \subsection{Dataset considerations}
        ...
        \end{figure}
        \par{
            In order to keep proper proportions and spatial information, replicas of surfaces are built only using simmetries. Therefore, from a single image other three are created. An example of such operation is shown in \ref{fig:data-augmentation}.
        }
        \par{
