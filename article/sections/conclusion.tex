...
\begin{table}
    ...
    \end{tabular}
    \vspace{0.25cm}
    \caption{Comparison between performances obtained by different class detectors, using or not both the MinMax normalization and the equalization.}\label{table:bayesopt-params}
\end{table}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{graphics/results/detector-result-class1}
...
\end{figure}
	\par{
		...
		\mathcal{A} = \frac{\lvert X \cap Y \rvert}{\lvert Y \rvert}.
		\end{equation*}
		The average accuracies are reported in \emph{Table \ref{fig:segmentation-test}}. In \emph{Figure \ref{fig:segmentation-test}} accuracies distribution for each detector class are reported. 
	}
