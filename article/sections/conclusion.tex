...
\end{table}
\par{
	In \emph{Table \ref{table:test-bayesopt}} are reported the average loss and accuracy obtained by the trained \emph{detectors}, either using or not the equalization and the MinMax normalization on the images. Recall that the defects were localized with bounding boxes. Hence, overhead pixels were intrinsic to the implementation and, thus, the loss function had a non-null lower bound.
}
\par{
	It is clear that in both the equalized and non-equalized settings, the MinMax normalization improves the performance of each \emph{detector}. Indeed, although the accuracy increases using normalization, the loss value increases too. This means that the localization of more defects is not due to an increase in the \emph{detector} precision, but only to larger bounding boxes and, thus, more overhead pixels.
}
\begin{table}
	\centering
	\normalsize
...
\par{
	In \emph{Table \ref{table:params-bayesopt}} the optimal parameters values are reported. It is visible that tuned parameters reflect the shape of defects studied in \emph{Section \ref{subsection:defects}}. In \emph{Figures \ref{fig:detector-results-1}, \ref{fig:detector-results-2}, \ref{fig:detector-results-3}} and \emph{\ref{fig:detector-results-4}} are shown some examples of localization of defects from classes No.$1$, No.$2$, No.$3$ and No.$4$ respectively. In \emph{Figure \ref{fig:segmentation-test}} the test accuracies distribution for each \emph{detector} are reported. 
}
\begin{figure}
	\centering
...
\end{figure}
\par{
	The distribution of accuracies in the equalized setting (\emph{Figure \ref{fig:segmentation-test-eq}}) differs greatly from the non-equalized one. Indeed, there is a larger amount of images with values of accuracies near zero. Average accuracies of only $0.49$, $0.33$, $0.46$ and $0.55$ were reached for classes No.$1$, No.$2$, No.$3$ and No.$4$, respectively.
}
\par{
	The worsening of the results after equalization is probably due to the usage of a global (i.e. over all the image domain) equalization procedure. Indeed, noise is greatly increased by this equalization approach, wherever there are wide and nearly monochromatic regions (e.g., black backgrounds). Examples of these cases are shown in \emph{Figure \ref{fig:segmentation-test-eq}}. This was obviously detrimental to the optimization procedure. However, the optimal values found are reported in \emph{Table \ref{table:params-bayesopt-eq}}.
}
\par{
	Although the accuracies of the \emph{detectors} on equalized images were lower, one can infer from the obtained losses that the proportion of overhead pixels is lower too. Hence, equalized \emph{detectors} seems to be more precise. This is also confirmed by the performance of the \emph{classifier}, which are superior on equalized images. Conceptually, the equalized images have higher contrast, and thus an edge detector is expected to identify better abrupt changes.
}
\par{
	These results suggest that higher performances could be obtained by the \emph{detectors} using a proper equalization procedure, dealing finer with the situations with non-negligible noise.
}
