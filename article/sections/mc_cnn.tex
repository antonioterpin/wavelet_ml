    ...
    | g h i |              | 0 0 0 0 0 |
     -     -               | g 0 h 0 i |
        ...
        \begin{figure}
            ...
            \caption{Learned features with a traditional convolutional layer (top) and with the \emph{spreader} (bottom). There are $4$ tiled features in both cases.}\label{fig:learned-features-hope-net}
        \end{figure}
        \par{
            In figure \ref{fig:learned-features-hope-net} the learned features of four $3\times3$ filters of a traditional convolutional layer with padding $\left[673\;1\right]$ are juxtaposed to the ones learned by four $3\times 3$ filters with a dilation factor of $\left[400\;400\right]$ and proper padding $\left[672\;0\right]$. 
        }
        \par{
        ...
        \par{
            It would eventually learn to handle large original pictures. As an example, if the dataset is composed by a cornucopia of large defects, the \emph{spreader} could copy only vertically the defect, using only $3$ of the $9$ available weights.
        }
        \par{
