\section{Detector}\label{section:region_proposals}
    ...
    \subsection{Edge detection}\label{subsection:contour_detection}
        ...
        \par{
            The hysteretic parameters \emph{low-threshold} $T_{low}$ and \emph{high-threshold} $T_{high}$ have been tuned through Bayesian optimization, together with the parameters of the Kovesi algorithm.
        }
    	\par{
...
bounding_box =  | maxX maxY |
    	...
    	\par{
    		ACCURACY MEASURE
    		\begin{equation*}
	    	A\left(X, Y\right) = \frac{\lvert X \cap Y \rvert}{\lvert Y \rvert}
    		\end{equation*}
    	}
	    \par{
		...
		\begin{figure}
			...
			\includegraphics[width=\linewidth]{graphics/architecture/detector_ex_hysteresis}
			\vskip .05cm
			\caption{Detector results on a sample image. DA FARE CON }
			\label{fig:detector-example}
		\end{figure}
