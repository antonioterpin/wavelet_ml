\section{Detector}\label{section:region_proposals}
    ...
    \subsection{Edge detection}\label{subsection:contour_detection}
    	...
    	\begin{figure} 		  
    		...
    		\vskip .05cm
    		\caption{Output of the edge detector on a sample image (first from the top) using different thresholds $(T_{low},T_{high})$. From top to bottom: $(30,50)$, $(60,100)$ and $(100,150)$. Default parameters of the Kovesi algorithm were used.}
...
bounding_box =  | maxX maxY |
        ...
        \par{
            As a first approximation, the implementation of the \emph{detector} was done to output the bounding boxes instead of the alpha shape. Therefore, the $X$ set in \emph{Section \ref{subsection:segmentation}} was characterized by all the pixels inside the bounding box of the alpha shape regions.
        }
        \begin{figure} 		  
    		\includegraphics[width=\linewidth]{graphics/architecture/detector-ex}
    		\vskip .05cm \includegraphics[width=\linewidth]{graphics/architecture/detector-ex-segmentated}
    		\vskip .05cm \includegraphics[width=\linewidth]{graphics/architecture/detector-ex-bounding-box}
    		\vskip .05cm
    		\caption{ROIs (middle) and relative bounding boxes (bottom) obtained on an example surface (top).}
    		\label{fig:segmentation-bb-example}
    	\end{figure}
    	\par{
    		In \emph{Figure \ref{fig:segmentation-bb-example}} examples of ROIs extracted from a steel surface picture are illustrated. In the picture, also the ROIs bounding boxes are shown.
    	}
